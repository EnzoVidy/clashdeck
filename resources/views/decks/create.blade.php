            ...
            <div class="form-container">
                <form method="POST" action="{{ route('decks.store') }}">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                        <h3 class="section-title" style="margin:0;">Sélectionnez exactement 8 cartes</h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateState() {
                ...
                // Bloquer ou débloquer les autres
                checkboxes.forEach(cb => {
                    if (!cb.checked) {
                        // Si on a atteint la limite, on désactive les cases non cochées
                        cb.disabled = checkedCount >= limit;

                        // Ajoute un effet visuel sur le parent (optionnel, géré par CSS aussi)
                ...
                // Changer la couleur du compteur si complet
                if (checkedCount === limit) {
            ...
            // Ajouter l'écouteur sur chaque case
            checkboxes.forEach(cb => {
                cb.addEventListener('change', updateState);
            });

            // Lancer une fois au chargement (pour gérer les old inputs)
            updateState();
        });
    </script>

    <style>
        /* CSS GLOBAL (identique précédent) */
        ...
        /* --- NOUVEAU : ETAT DÉSACTIVÉ (quand on a 8 cartes) --- */
        .card-option input:disabled + .card-visual {
            opacity: 0.5;                /* Rendre transparent */
            filter: grayscale(100%);     /* Rendre gris */
